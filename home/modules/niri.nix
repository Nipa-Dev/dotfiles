...
in {
  ...
  programs.niri.settings = {
    ...
    spawn-at-startup = [
      {command = ["waybar"];}
      {command = ["swww-daemon"];}
      {command = ["swww" "img" "~/wallpapers/fractal-1920x1080.png"];}
    ...
    binds = {
      ...
      "XF86AudioRaiseVolume" = {
        allow-when-locked = true;
        action.spawn = ["pactl" "set-sink-volume" "@DEFAULT_SINK@" "+1%"];
      };
      ...
      "Shift+XF86AudioRaiseVolume" = {
        allow-when-locked = true;
        action.spawn = ["pactl" "set-source-volume" "@DEFAULT_SOURCE@" "+5%"];
      };
